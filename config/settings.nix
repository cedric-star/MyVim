  ...
  config = {
    extraConfigLuaPre = ''
      ...
      vim.diagnostic.config({
        underline = true,          -- die rote Unterstreichung
        virtual_text = true,       -- Text rechts neben der Zeile (optional)
        signs = true,              -- Icons in der Signalleiste (hast du schon)
        update_in_insert = false,  -- nicht während dem Tippen updaten
        severity_sort = true,      -- schwerste Fehler zuerst
      })
    ...
    extraConfigLua = ''
      ...
      -- Neo-tree Mappings mit Verzögerung setzen
      vim.api.nvim_create_autocmd("FileType", {
        ...
        callback = function()
          ...
          vim.notify = function(msg, level, opts)
            if type(msg) == "string" and msg:find("Invalid mapping") then
              return
            end
